...
export default function EntriData() {
  ...
  const { filePdf } = useStore(entri_dataStore);
  const {
  ...
  useEffect(() => {
    if (id) {
      ...
      fetchDataHardCopy(id);
    }
  }, [id, fetchDataHardCopy]);

  // useEffect(() => {
  //   fetchDataHardCopy(136);
  // }, [fetchDataHardCopy]);
  ...
  const filteredData = verifikasi_dokumen_hardcopy.filter((item) =>
      ...
      <div className="space-y-3 pt-8">
        ...
        <div>
          {filePdf ? (
            ...
            <p className="text-red-500">PDF tidak tersedia.</p>
          )}
        </div>
