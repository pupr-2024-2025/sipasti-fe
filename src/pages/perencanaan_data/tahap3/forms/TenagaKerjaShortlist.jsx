...
const TenagaKerjaShortlist = ({ values, rows, hide }) => {
  const tabs = ["Material", "Peralatan", "Tenaga Kerja"];

  const { setCurrentTab } = useTahap3Store();

  return (
    <div className={`${hide ? "hidden" : ""}`}>
      <div className="space-y-4">
        <div className="flex flex-row justify-between items-center">
          <SimpleTabs items={tabs} onChange={setCurrentTab} selectedValue={2} />
      ...
      <div
        className={`rounded-[16px] border border-gray-200 overflow-hidden mt-6`}>
              ...
              {rows.map((item, index) => (
                <tr
                  ...
                  <td className="px-3 py-6 text-sm text-center">
                    ...
                    <Field
                      type="checkbox"
                      name={`tenaga_kerja[${index}].checked`}
                      className="h-4 w-4">
                      {({ field, form }) => (
                        <Checkbox
                          {...field}
                          onChange={(e) => {
                            form.setFieldValue(`tenaga_kerja[${index}]`, {
                              value: item.id,
                              checked: e,
                            });
                          }}
...
export default TenagaKerjaShortlist;
